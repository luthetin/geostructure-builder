...
用假 WebGL + 假 DOM 让程序在 Node 里完整跑起来，然后做数值断言。共 **168 项**，覆盖：
...
- **模型命名**：输入名字后写进状态与页面标题；`snapshot()` 里带 `name`；存→读回来不丢（输入框也要跟着更新）；下载文件名用它（`a/b:c*d` → `a_b_c_d`），没起名回落到"地层模型"；旧存档没有该字段也能读
- **自动等高距**：三种量级的起伏（60 / 300 / 3800 m）各点一次「自动」，断言线数都落在 6~20 条- **基底也服从规则**：基底切面上"透明"与"z > 该柱最低的更高界面"逐顶点一致（不符 0/1152），被占住的那一截 429 个顶点全透、没被占住的 723 个全实心；基底上沿与它上方那个地层的底面同源（不符 0/96）；基底深度改成 900 m 结论不变
