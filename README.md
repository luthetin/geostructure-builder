...
每张层面是一个独立的薄面（沿法线抬高 1.5 m，免得和地层的顶/底面 z-fighting），有自己的颜色与透明度。**调某一张的透明度，就能看清层面与层面之间的空间关系。**

但**它照样服从层序规则**：`S_i = min( ①min(z_j − z_i) over j>i , ②min(z_i − z_j) over j<i )`，`S < 0` 的地方这片薄面不画。曾一度把它改成整片无条件画出来，结果老界面的薄面跑到更年轻的界面之上（实测 8516 个被压住的柱子薄面全还画着），所以改回来。

这条 `min` 也用**软最小值**（和岩体那条 `C_k` 同一个道理、同一个尺度）：硬 min 在"最小值换主人"的地方会折一下（实测这个模型上每个界面换 75~217 次），丢边界跟着拐折。软 min 恒 ≤ 硬 min，所以**只会多透明、绝不会多显示**，层序规则不会被破坏。
...
用假 WebGL + 假 DOM 让程序在 Node 里完整跑起来，然后做数值断言。共 **154 项**，覆盖：
...
- **交界面这条规则也用软最小值**：断言软场**恒 ≤ 硬 min**（只会多透明、绝不会多显示），且多透出来的部分落在软化带内（不超过一个网格步长 × log 界面数）；另有一条对照断言：叠置正常、没有"换主人"的模型上，透明判据仍**逐点等于定义**
