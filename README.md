...
**侧壁（切面）用的是同一个场的连续版本，所以两端和顶/底面严格一致。**

顶面和底面是"面"，在每一点上判一次；侧壁是一段高度，要在**整段**上判。同一个规则的连续写法，是看某个高度离控制它的那个更高界面有多远：
...
- **底面透明 ⇒ 侧壁下沿透明**，**顶面透明 ⇒ 侧壁上沿透明** —— 两端严格一致，俯视图透的地方侧面也透
- 只有一端为负时，透明区是一条**楔形**，在相切/尖灭处收拢到零
- 两端都为负时整条贯穿（那时上下两个界面确实都被埋掉了）
- 两端都不为负（含恰好相切）时**保持实心**

上沿那个 `min(0, ·)` 是关键。最上面那个界面没有更高的界面压着，它的场是 `+∞` 哨兵；但几何上它到自己的间距恰好是 **0**，所以取 0 是有依据的，不是凑出来的数。（曾经用一个"模型高程跨度"的自由尺度硬凑，那是错的：**0 才是 f 在 z = z_{k+1} 处的真值**。）

这个判据走过三个弯路，都记在这里免得再走回去：

| 写法 | 后果 |
|---|---|
| 上下沿都用 `min(下界面场, 上界面场)`，整柱一个值 | 面和侧壁是一致的（这点是对的），但柱内没有梯度，相切附近会整条透明成一个**洞** |
| 侧壁完全不参与规则 | 规则在切面上彻底看不见 |
| 改用"这一柱的厚度是否 < 0"判 | 判据和顶/底面**不再是同一个**了 —— 于是出现**俯视图透明、侧面却实心** |

**关键教训：侧壁的场必须和顶/底面同源。** 一旦给侧壁换一个别的判据（不管是厚度、还是"永远实心"），就会出现"俯视图透了、侧面没透"这种不一致。
...
用假 WebGL + 假 DOM 让程序在 Node 里完整跑起来，然后做数值断言。共 **130 项**，覆盖：
