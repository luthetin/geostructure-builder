...
**约束「最高的那个永不透明」。** 这是②带来的新问题：地表扎下去以后，**被它扎穿的那个老界面反而成了该处最高的面**，也就是露出来的那个地表面。这时候①会在它身上触发（有个更年轻的界面在它下面），把它也丢掉 —— 于是那个位置**一个面都没有了**：本该露出下面地层的颜色，结果露出来的是"透明"。所以要加这条约束：最高的那个界面是这一处的地表面，把它丢掉就没有面了。

三条合起来的效果，正是想要的那张图：

| 位置 | 结果 |
|---|---|
| 叠置正常处 | 只有最上面那个面不透明，其余照旧 —— 行为完全不变 |
| 地表扎穿中界面处 | 地表判透明（不再盖着），**被剥露出来的那个面不透明**，看到它的颜色 |
...
用假 WebGL + 假 DOM 让程序在 Node 里完整跑起来，然后做数值断言。共 **146 项**，覆盖：

- **数学正确性**：Catmull-Rom 严格穿过控制点（误差 5.6e-8 m）；抛物面的高斯曲率与解析值 c² 相对误差 **0.000%**；穹隆/鞍部/圆柱面的曲率符号正确（圆柱面 K 精确为 0）
- **规则正确性（两半 + 一条约束）**：①"更年轻的压在它下面"逐点核对；②"更老的压在它上面"—— 这一半是唯一能让**最上面那个界面**被判透明的途径；约束"当下最高的那个界面永不透明"逐点核对（**4656 个被扎下去的点全部保持不透明**，扎下去的那个 4656 点全部判透明）。另断言：**叠置正常时②一次都不触发**（0 个透明顶点），且**被剥露出来的那层顶面 0/96 柱透明**（不是整片透掉）
