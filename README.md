...
- **初始相机距离按画面宽高比自适应**：固定 `图幅×1.95` 是按横屏调的，手机竖屏水平视野只有一半左右，模型会横向溢出 —— 竖屏时按水平视野重算（实测桌面 7800 → 手机竖屏 13981）**内置预设**
- 面板顶部有「预设」下拉，可选一个内置示例模型（当前：**预设1** —— 19 个交界面 / 18 个地层的柱状模型）。选中即载入，名称、颜色、显隐、等高距、视角一并恢复
...
用假 WebGL + 假 DOM 让程序在 Node 里完整跑起来，然后做数值断言。共 **195 项**，覆盖：
...
- **内置预设**：假 DOM 直接从 HTML 里抓出 `script.preset` 块当作节点，所以验的是**真·内嵌数据**：JSON 能解析、界面/地层数与源文件一致、下拉里有选项、选中后模型真的换掉（19/18）、名称与等高距一并恢复- **触屏交互**：在假 DOM 上派发 touch 事件，验证单指拖动改高度、松手后不再拖动、双指张开拉近相机、多选/整体升降两个开关（含互斥与文案）、以及 ☰ 的三段循环与遮罩收起
