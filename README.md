...
**带与带的颜色边界是锐利的，而且精确落在交线上**：颜色不在顶点上混，而是逐顶点只传一个**带值**，片元里按最近邻查一张 1×N 的调色板取色（`floor(带值+0.5)` 取最近的那个带）。逐顶点涂色的话，交界那一格的颜色是线性混过去的，过渡带**整整一格宽（≈42 m）**，画在那里的交线窄带盖不住，两边各露出一截毛边 —— 那才是"表露面有锯齿"的真正原因。

但只用整数带号还不够：阈值卡在带值 ±0.5 处，插值跨过它的地方只能是**格边中点**，而真实交线一般不在中点 —— 实测偏出**中位 10.7 m、最大 20.6 m**（半格 20.8 m），露出来的仍是一条锯齿。所以跨带的那条格边上要把顶点往边界挪一点。设交线在该边 **t** 处（从低带那一端算起，分界界面号 = 两带中小的那个）：

```
t ≤ 1/2：带值_低 = e + 0.5 − 0.5t/(1−t)，带值_高 = e + 1
t > 1/2：带值_低 = e − 0.5，             带值_高 = e + 1.5 − (2t−1)/t
```

两种取值都仍在各自的 `[band−0.5, band+0.5)` 内，带号本身不会被改坏；一个顶点被多条跨带边约束时取平均。修正后实测偏差 **0.000 m** —— 颜色边界与交线窄带重合在同一条曲线上，窄带于是回到细线宽度，不需要靠加宽去盖误差。

**调色板必须跟着模型重建**：它只在 `if (!PAL.tex)` 时建过一次的话，真机会把调色板永远停在开屏那一版 —— 之后打开地层更多/配色不同的模型，更靠后的带值会被 `CLAMP_TO_EDGE` 夹到最后一格，**整个表露面变成一种颜色**。所以 `rebuild()` 里无条件重建它。

**唯一例外是曲率显示**：调色板在片元里直接取色，会把地表面逐顶点算出来的曲率色带整个盖掉，所以打开曲率时地表面退回逐顶点色（曲率是连续场，本来就该插值），关掉再走回调色板。
...
用假 WebGL + 假 DOM 让程序在 Node 里完整跑起来，然后做数值断言。共 **163 项**，覆盖：
...
- **表露面的颜色不插值**：地表面那张顶面必须带上逐顶点的**露头带号**缓冲（带号落在调色板范围内、且至少出现两种），并标了"走调色板"；渲染后**调色板纹理确实建起来了**（项数 ≥ 2）。颜色一旦退回逐顶点插值，两带交界那一格就会混成一整格宽的毛边，这组断言会直接挂掉
- **曲率显示不被调色板盖掉**：调色板在片元里直接取色，会把地表面逐顶点算出的**曲率色带**整个盖住，所以打开曲率时地表面必须退回逐顶点色。造一个穹隆（K 到处不为零，实测 max|K| = 6.2e-7）断言：曲率关着时走调色板，打开后不走，且**顶点色与露头带色确实明显不同**（色差 0.41，色带真画上去了），关掉后又走回调色板
- **调色板跟着模型重建**：只建一次的话真机会把调色板停在开屏那一版，整个表露面变成一种颜色。断言换模型后调色板项数 = 地层数+1，并且**逐分量核对接收到的那张纹理**就是"基底 + 各地层自身颜色"（不是旧模型那一版）。这条能测出来，前提是假 GL 的 `createTexture()` 返回真值 —— 返回 undefined 会让"只在没建过时建一次"退化成"每次都重建"，把 bug 完全遮住，所以假 GL 也记下每次 `texImage2D` 的内容
- **表露面的颜色边界落在真实交线上**：逐条跨带格边，把"阈值跨过处"和"场的零交点"都算出来比位置 —— 只传整数带号时偏出中位 10.7 m、最大 20.6 m（就是那条锯齿）；修正后 **0.000 m**。另有断言保证修正后的带值不越出各自带号的 ±0.5（带号不会被改坏）
