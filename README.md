...
- ⚠ **但复用的只是形状，透明场必须每帧重算并重传**：网格里烘着"哪里该透明"这个逐顶点场，它依赖**所有**界面的高程，一直在变。所以复用网格时走一条「只更新场」的快路径 —— 重算场、只重传这一个 float 属性（21122 个数 / 84 KB），而不是整块网格重建。
  顶点对应哪个格点记在 `m._q` 里（0xFFFF = 永远画：地表顶盖与切开的多边形）。
  这一条是用户报的第二个 bug 的根因：网格复用了、场没更新，于是"拖动时规则失效，撒手才正常"。
...
用假 WebGL + 假 DOM 让程序在 Node 里完整跑起来，然后做数值断言。共 **213 项**，覆盖：
