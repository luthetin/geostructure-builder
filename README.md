...
> 面是透明的，岩体是实心的；**一个界面之上只出现层序比它高的岩层**，颜色按交线一层一层地换。
...
```
地表面 env(x,y) = max over 全部界面 of 界面高程      ← 上包络

顶面 / 底面（"面"）：场 = min( env − 顶点高程 , 本层厚度 )
四周侧壁（剖面）  ：场 = 本层厚度
```

**为什么面要减 `env`。** 规则（一个面之上只能显示层序更高的岩层）管的是**面上方的东西**，不是面自身。所以：

- 一张面高过地表面（等于它在岩石内部）→ 这张面没有意义 → 不画。这正是"地表层之上冒出别的岩层"的根源：最上面那层尖灭之后，它的顶面本来会留在岩石内部当中
- 本层尖灭（厚度 < 0）→ 这张面也不存在 → 不画

**为什么侧壁只减厚度、不减 `env`。** 侧壁是**剖面**：规则让面上方的东西透明，但**面本身、以及面之间的岩石要正常显示** —— 就像野外剖面上看到的岩层条带不是透明的。之前把 `env` 那一项也加到侧壁上，剖面上就空了一块（实测：97 列里 45 列有洞、占总高 24.6%）。改成只用厚度之后：空洞占整面 **0.42%**，单列最大 15.8 m，全是相邻层界面处网格间过零点留下的细缝。同时各层切面的覆盖从 `89/97、79/97、52/97` 变成 `97/97、97/97、52/97`（52/97 是那层自己尖灭掉的列，正确）。

**不变量：画出来的顶点一律不高过 `env`** —— 地表之上不许有东西。逐顶点核对 34810 个顶点越界 **0** 个、最高 **0.0 m**。

**露头带从哪来。** `env` 是上包络，每一条带就是某个界面正好成为包络的那片区域，**带与带的分界就是两条界面的交线**。实测俯视：**地层1 的颜色 4262 柱 / 地层2 的颜色 5147 柱，空白 0 柱，带分界 181 条** —— 一层一层地换，没有洞。
