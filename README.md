...
**侧壁（切面）的判据是"这一柱地层有没有倒置"。** 侧壁上判透明的依据**不是**界面自己的场，而是**这一柱的厚度**：

```
厚度 = 上面那个界面的高程 − 下面那个界面的高程
厚度 ≥ 0  → 层序正常（含恰好相切、尖灭）→ 切面实心
厚度 < 0  → 老的地层跑到新地层上面去了 → 这一柱岩石根本不存在 → 整条厚度透明
```

为什么必须这样分：

- **相切 / 尖灭**是厚度趋近 0 但没变负，层序没颠倒 —— 岩层还在，切面就该实心
- **交错 / 倒置**是厚度真的为负，老地层压在年轻地层上 —— 这一柱是假的，如果不透明，它的切面就会跟相邻地层的切面**叠在一起**，看着就是几片面互相穿插

场沿柱子是**常数**，因为"这一柱是不是倒置"本来就是整个柱子的事，不是柱子内某个高度的事。

这个判据走过两个弯路，都记在这里免得再走回去：

| 写法 | 后果 |
|---|---|
| 上下沿推同一个 `min(下界面场, 上界面场)` | 只要**任意一个**相邻界面被埋就整条透明 —— 会在**没有倒置**的地层上挖洞（实测：一个正常地层 96 个柱子里有 60 个被误判透明） |
| 侧壁完全不参与规则 | 规则在切面上彻底看不见 |
| 上下沿各推"自己那个界面的场"，中间插值 | 透明区成了楔形，但"倒置"是全柱的事，楔形没有依据；而且最上面那个界面的场是 `+∞` 哨兵，插值后过零点被压掉，等于看不见 |

**只有顶面和底面的"面"是逐点的，侧壁是逐柱的。** 同一个规则在零维和一维上的两种落地方式，不是两套规则。
...
用假 WebGL + 假 DOM 让程序在 Node 里完整跑起来，然后做数值断言。共 **133 项**，覆盖：

- **数学正确性**：Catmull-Rom 严格穿过控制点（误差 5.6e-8 m）；抛物面的高斯曲率与解析值 c² 相对误差 **0.000%**；穹隆/鞍部/圆柱面的曲率符号正确（圆柱面 K 精确为 0）
- **规则正确性**：交界面的透明判据**逐点**等于定义（9409 个顶点不符 0 个）；交界面透明的地方地层底面逐点跟着透明；规则是单边的
- **透明只从规则来**：单独把交界面透明度调低，上下地层的透明度**一字不变**
- **侧壁的判据是"这一柱有没有倒置"**：在同一个模型上同时造出"倒置""正常""恰好相切"三种柱子，逐列核对 —— 倒置的 59 柱全部整条透明、正常的 36 柱全部实心、贴着相切线的那一柱（厚度 0.0 m）保持实心；用重心坐标**精确**算出 +x 侧壁的透明面积占比 62.5%，正好等于倒置柱子的比例。**同一条模型上，地层 0 自己从不倒置，所以它的切面 96 柱全部实心** —— 而旧的 `min` 判据会把其中 60 柱误判成透明（这就是"切面上出现不该有的洞"的根因）
