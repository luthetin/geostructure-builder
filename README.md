...
**只有顶面和底面参与这个规则，侧壁（切面）永远实心。** 这条很要紧：如果不加区分地把场算到侧壁上，那么地层**尖灭**（两侧厚度趋近于零）的地方，上下两个界面几乎相切，场在那里必然过零 —— 于是切面上被挖出一片透明的洞。地质上这是错的：切面是"被切开的岩石"，它是实心的，透明与否是**平面图上层面的可见性**问题，跟切面无关。所以侧壁的场一律写成"永远可见"。
...
用假 WebGL + 假 DOM 让程序在 Node 里完整跑起来，然后做数值断言。共 **126 项**，覆盖：
...
- **透明只从规则来**：单独把交界面透明度调低，上下地层的透明度**一字不变**（回归测试，防止面板语义和规则语义又粘到一起）
- **侧壁永远是实心的**：在上下界面相切（地层尖灭）的构造上，顶底面按规则出现 9312 个透明顶点，而 2304 个侧壁顶点**一个都不判透明**
