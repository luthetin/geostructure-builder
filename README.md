...
**带与带的颜色边界是锐利的**：颜色不在顶点上混，而是逐顶点只传一个**带号**，片元里按最近邻查一张 1×N 的调色板取色（`floor(带号+0.5)` 取最近的那个带）。逐顶点涂色的话，交界那一格的颜色是线性混过去的，过渡带**整整一格宽（≈42 m）**，画在那里的交线窄带盖不住，两边各露出一截毛边 —— 那才是"表露面有锯齿"的真正原因。

代价是颜色边界落在**两带带号取中的位置**（跨接那一格的中点），与真实交线最多差**半个网格步长（≈21 m）**。所以**涉及地表的交线**那条窄带单独加宽到 `max(±8 m, 0.55 格)`，把这个位置偏差连同交线本身一起盖住；其余交线仍是细线。
...
用假 WebGL + 假 DOM 让程序在 Node 里完整跑起来，然后做数值断言。共 **157 项**，覆盖：
