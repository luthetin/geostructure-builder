...
> **一个界面之上，只能出现层序比它更高的岩层。**

写成场就是：

```
C_k(x,y) = min over i>k of （界面 i 的高程）     比本层年轻的界面里最低的那个
每个顶点的场 = C_k − 顶点自身高程；场为负就不画
```

这一条把之前几版补丁**全部包含**了，不用再加别的：

| 情形 | 为什么自动成立 |
|---|---|
| 本层尖灭（厚度 < 0） | `C_k ≤ 界面 k+1 的高程`，所以底面的场 ≤ 厚度，厚度为负时底面也判负 → 整层消失 |
| 地表之上不许有东西 | 地表是 `i>k` 里的一员，所以 `C_k ≤ 地表高程`；顶点高过地表就判负 → 剪掉 |
| 最上面那层的顶面是盖子 | 那一层只有 `i=n−1` 一个参照，`C = 地表高程`，顶面的场恰好是 0 → 永远画着，地表天然完整 |

`C` 是连续的，所以所有边界（尖灭线、裁剪线）都**精确落在界面的交线上**，不会有网格锯齿。

**实测（截图里那个真实模型）**：4 个界面 / 3 个地层，中界面最高拱到 4726 m 而地表在那一带只有 1181 m。改之前画出来的 52364 个顶点里有 **10457 个在地表之上、最高伸出 3037 m**；现在越界 **0 个，最高 0.0 m**。同时逐柱核对"画出来的顶点一律不高过 C_k"：21122 个顶点越界 **0**。
