...
```
厚度 = ifaces[k+1] 的高程 − ifaces[k] 的高程
厚度 ≥ 0 → 这一层存在 → 实心（顶面、底面、侧壁全都实心）
厚度 < 0 → 已经尖灭掉了 → 整层不画
```

每个顶点的场就**直接写成这一层的厚度**。厚度是连续的，所以尖灭边界精确落在相切线上，侧壁在相切处自然收拢成零宽，不会留一截孤零零的墙。

**为什么规则不作用到地层上。** 一个交界面会被**上下两个地层同时当作自己的边界**：`iface k+1` 既是地层 k 的顶面、又是地层 k+1 的底面。如果把界面的判据套到地层面上，这两个共用同一张面的地层就会得到相反的结论 —— 一定是"本该消失的地层又出现了"，或者"露出来的地层是透明的"，怎么调都会错一边。分开之后互不干扰：**面是透明的（规则管），岩体是实心的（存在性管）**。
...
| 深扎到下界面以下 | 中界面那张面也透明 | 中界面上下两层都按各自厚度判 |
...
- **地层实体的每个面都用同一个场（本层厚度）**：逐顶点核对（不符 0/6912），并逐柱断言顶面 / 底面 / 侧壁**三者同进同退** —— 整层存在或整层不画，不会出现"一半透一半不透"
